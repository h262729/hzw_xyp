...
import com.hzw.xyp.base.tools.BeanTools;
...
public class BaseRepositoryImpl<T, ID extends Serializable> extends SimpleJpaRepository<T, ID> implements BaseRepository<T,ID> {
    ...
    public int updateAllByProperties(String name, Object value, Object... keyAndValues) {
        // 更新hql语句
        String hql = "UPDATE " + domainClass.getSimpleName() + " t SET t." + name + " = :" + name;
        hql += this.createWhere(keyAndValues);  // 获取where条件语句
        // 参数
        Map<String, Object> params = this.getWhereParams(keyAndValues);
    ...
    private List<T> getBeans(Object[] keyAndValues, Integer maxCount){
        ...
        hql += this.createWhere(keyAndValues); // 条件语句拼接
        Map<String, Object> params = this.getWhereParams(keyAndValues); // 获取条件参数
     ...
     * @return
     */
    private String createWhere(Object[] keyAndValues){
        if(keyAndValues == null || keyAndValues.length%2 != 0){
            return "";
        }
     ...
     * @return
     */
    private Map<String, Object> getWhereParams(Object[] keyAndValues){
        if(keyAndValues == null || keyAndValues.length%2 != 0){
            return new HashMap();
        }
        ...
        for(int i = 0; i < keyAndValues.length; i+=2){
            ...
            String key = keyAndTabStr.split("\\$")[0]; // 字段名
